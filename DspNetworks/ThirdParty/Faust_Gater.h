...
#include "src/Faust_Gater.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct Faust_GaterMetaData {
		SN_NODE_ID("Faust_Gater");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using Faust_Gater = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _Faust_Gater, Faust_GaterMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
